...
const getFile = async (token, itemId, columnId) => {
  try {
    const mondayClient = initMondayClient();
    mondayClient.setToken(token);
    ...
    const response = await mondayClient.api(query, { variables });
    const files = JSON.parse(response.data.items[0].column_values[0].value).files;
    console.log(files)
    const assetIds = files.map(file => file.assetId)
    console.log(assetIds)
    const photo = await mondayClient.api(
      ...
      { variables: { columnId, itemId, assetIds } }
    );
    console.log(photo)
  } catch (err) {
    console.error(err);
